...
namespace Gothic_II_Addon {
  ...
  public:
    ...
    T& GetSafe( int nr ) const {
      if( numInArray <= 0 ) {
        Common::Message::Fatal( "D: zTree(zCArray::GetSafe): numInArray<=0" );
      }
      if( numAlloc <= 0 ) {
          Common::Message::Fatal( "D: zTree(zCArray::GetSafe): numAlloc <=0" );
      }
      if( nr < 0 ) {
        nr = 0;
      }
  ...
  public:
    ...
    T& GetSafe( int nr ) const {
      if( numInArray <= 0 ) Common::Message::Fatal( "D: zTree(zCArraySort::GetSafe): numInArray<=0" );
      if( numAlloc <= 0 ) Common::Message::Fatal( "D: zTree(zCArraySort::GetSafe): numAlloc <=0" );
      if( nr < 0 ) {
        nr = 0;
      }
      else if( nr >= numInArray ) {
